...
namespace AutoAsync.Helpers
{
	...
	class SilverlightCustomAttributeBuilderCreator
	{
		public static CustomAttributeBuilder GetAttributeBuilder(
			Attribute attribute,
			Action<List<PropertyInfo>, List<object>> onModifyProperties)
		{
			...
			if (onModifyProperties != null)
				onModifyProperties(properties, propertyValues);

			return new CustomAttributeBuilder(ctor, ctorArgs,
				properties.ToArray(), propertyValues.ToArray(), fields, fieldValues);
		}
		...
		private static List<object> GetPropertyValues(
			Type attType, out List<PropertyInfo> properties, Attribute original, Attribute replicated)
		{
		...
		private static PropertyInfo ReplaceIfBetterMatch(
			ParameterInfo parameterInfo, PropertyInfo propertyInfo, PropertyInfo bestMatch)
		{
